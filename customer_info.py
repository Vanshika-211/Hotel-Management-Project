...
class CustomerInfo:
    def __init__(self, root):
        ...
        def display_info():
            ...
            # Fetch customer data ordered by room_number
            cursor.execute("SELECT Fullname, room_number FROM Hotel ORDER BY room_number ASC")
            ans = cursor.fetchall()
    
            self.name_customer_entry.delete(1.0, END)  # Clear previous entries
            self.room_no_customer_entry.delete(1.0, END)  # Clear previous entries
    
            for i in ans:
                self.name_customer_entry.insert(INSERT, i[0] + '\n')
                self.room_no_customer_entry.insert(INSERT, str(i[1]) + '\n')
