...
class CheckOut:
    def __init__(self, root):
                    ...
                    with conn:
                        ...
                        for i in ans:
                            if room_number1 == int(i[1]):
                                ...
                                confirmation = messagebox.askokcancel("Confirmation", f"Check out {i[0]} from room {i[1]}?")
                                if confirmation:
                                    self.get_info_entry.insert(INSERT,
                                                            '\n' + str(i[0]) + ' has checked out from ' + str(i[1]) + '\n')
                                    with conn:
                                        cursor.execute("""DELETE FROM Hotel where room_number = ?""", [room_number1])
                                else:
                                    self.get_info_entry.insert(INSERT, "Check out canceled by user")
                else:
        ...
        self.check_out_button = Button(bottom, text="CHECK OUT", font=('', 15), bg="#15d3ba", relief=RIDGE, height=2,
                                    width=15, fg="black", anchor="center", command=check_out)
